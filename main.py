...
response_mercadolivre = requests.get(
    url_mercadolivre + produto, headers=headers).content

response_americanas = requests.get(
    url_americanas + produto, headers=headers).content
...
def buscar_produto(item):
    ...
    produtos_americanas = soup_americanas.findAll(
        "div", class_="col__StyledCol-sc-1snw5v3-0 jGlQWu src__ColGridItem-sc-122lblh-1 cJnBan")
    ...
    for produto in produtos_mercadolivre:
        ...
        nome_produto_mercadolivre = produto.find(
            'h2', class_="ui-search-item__title ui-search-item__group__element")
        # Extraindo preço do produto
        preco_real_mercadolivre = produto.find(
            "span", class_='price-tag-fraction')
        preco_cents_mercadolivre = produto.find(
            'span', class_='price-tag-amount').find("span", class_='price-tag-cents')
        if (preco_cents_mercadolivre):
            preco_mercadolivre = preco_real_mercadolivre.text + \
                ',' + preco_cents_mercadolivre.text
        else:
    ...
    for teste in produtos_americanas:
        # Extraindo o nome dos produtos
        nome_produto_americanas = teste.find(
            'h3', class_="product-name__Name-sc-1shovj0-0 gUjFDF")
        # Extraindo preço do produto
        preco_americanas = teste.find(
            "span", class_='src__Text-sc-154pg0p-0 price__PromotionalPrice-sc-h6xgft-1 ctBJlj price-info__ListPriceWithMargin-sc-1xm1xzb-2 liXDNM').getText().strip()
        site = "Americanas"
